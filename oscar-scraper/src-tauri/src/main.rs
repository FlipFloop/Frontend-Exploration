...
fn get_courses() -> String {
    ...
    for num in course_nums.iter_mut() {
        ...
        let row_element_data_selector = Selector::parse("td.dddefault, th").unwrap();
            ...
            for i in 0..rows.len() {
                if !rows[i].is_empty() {
                    course_info.push(Course {
                        num: num.0.clone(),
                        seats_capacity: rows[0].clone(),
                        seats_current: rows[1].clone(),
                        seats_remaining: rows[2].clone(),
                        waitlist_capacity: rows[3].clone(),
                        waitlist_current: rows[4].clone(),
                        waitlist_remaining: rows[5].clone(),
                    });
                }
            }

            for i in &course_info {
                if let Ok(result) = i.seats_remaining.parse::<u32>() {
                    if result > 0 {
                        num.1 = false;
                    }
                }
                if let Ok(result) = i.waitlist_remaining.parse::<u32>() {
                    if result > 0 {
                        num.2 = false;
                    }
                }
                ...
                println!("Class capacity: {}", i.seats_capacity);
                println!("Seats taken: {}", i.seats_current);
                println!("Seats left: {}\n", i.seats_remaining);
                println!("Waitlist capacity: {}", i.waitlist_capacity);
                println!("Waitlist spots taken: {}", i.waitlist_current);
                println!("Waitlist spots left: {}", i.waitlist_remaining);
